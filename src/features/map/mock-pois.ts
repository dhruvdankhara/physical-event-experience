...
import type { CreatePOIInput } from "@/repositories/poi.repository";

function locationForBlock(blockId: string, northMeters = 0, eastMeters = 0) {
  const block = findBlockById(blockId);
  if (!block) throw new Error(`Unknown stadium block: ${blockId}`);

  const coordinate = offsetCoordinateByMeters(block.coordinate, northMeters, eastMeters);
  return {
...
function locationForNearestGate(blockId: string, northMeters = 0, eastMeters = 0) {
  const block = findBlockById(blockId);
  if (!block) throw new Error(`Unknown stadium block: ${blockId}`);

  const nearestGate = findNearestGate(block.coordinate, block.preferredGateIds);
  if (!nearestGate) throw new Error(`Unable to find gate for block ${blockId}`);

  const coordinate = offsetCoordinateByMeters(nearestGate.coordinate, northMeters, eastMeters);
  return {
    sectionId: block.sectionId,
    blockId: block.id,
    location: toGeoJSONPoint(coordinate),
...
export const mockPOIs: CreatePOIInput[] = [
  {
